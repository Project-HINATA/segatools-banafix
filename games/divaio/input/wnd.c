...
static LRESULT WINAPI diva_io_wnd_proc(
            HWND hwnd,
            UINT msg,
            WPARAM wparam,
            LPARAM lparam);

HRESULT diva_io_wnd_create(HINSTANCE inst, HWND *out)
{
    HRESULT hr;
    WNDCLASSEXW wcx;
    ATOM atom;
    ...
    *out = NULL;
    ...
    wcx.lpfnWndProc = diva_io_wnd_proc;
    wcx.hInstance = inst;
    wcx.lpszClassName = L"divaIO";

    atom = RegisterClassExW(&wcx);

    if (atom == 0) {
        hr = HRESULT_FROM_WIN32(GetLastError());
        dprintf("divaIO: RegisterClassExW failed: %08x\n", (int) hr);
    ...
    if (hwnd == NULL) {
        hr = HRESULT_FROM_WIN32(GetLastError());
        dprintf("divaIO: CreateWindowExW failed: %08x\n", (int) hr);
